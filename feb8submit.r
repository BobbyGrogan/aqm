...
# manage the data
dfCA$county <- NULL
dfCA$grades <- as.factor(dfCA$grades)
dfCA$school <- NULL
dfCA$district <- NULL
...
model2 <- step(model1)
# gives summary of model2 with best subsets
summary(model2)

# 6: the best subset retained X, expenditure, income, english, income, and read
# 6: I disagree that the variable X would have any influence
...
model_test = lm(math ~ expenditure + income + english + read, data = training)
